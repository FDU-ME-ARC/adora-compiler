...
// CHECK: %[[R:.*]], %{{.*}} = ADORA.BlockLoad async {{.*}} %arg3 {{\[}}%{{.*}}] : memref<?xi32> -> memref<1xi32>
// CHECK: %[[Arow:.*]], %{{.*}} = ADORA.BlockLoad async {{.*}} %arg0 {{\[}}%{{.*}}, 0] : memref<?x24xi32> -> memref<1x24xi32>
// CHECK: %[[X:.*]], %{{.*}} = ADORA.BlockLoad async {{.*}} %arg1 [0] : memref<?xi32> -> memref<24xi32>
...
// CHECK: affine.load %[[Tmp]][0] : memref<1xi32>
// CHECK: arith.muli
// CHECK: arith.addi
